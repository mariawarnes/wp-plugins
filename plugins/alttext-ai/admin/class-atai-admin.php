...
class ATAI_Admin {
	...
	public function enqueue_styles() {
		wp_enqueue_style( 'atai-global', plugin_dir_url( __FILE__ ) . 'css/atai-global.css', array(), $this->version, 'all' );
	}
    ...
    wp_localize_script( $this->plugin_name, 'wp_atai', array(
      ...
      'security_edit_history'                   => wp_create_nonce( 'atai_edit_history' ),
  ...
  public function display_setup_notice() {
    if ( ! get_transient( 'atai_show_setup_notice' ) ) {
      return;
    }

    delete_transient( 'atai_show_setup_notice' );

    echo '<div class="notice notice--atai notice-info is-dismissible"><p>';

    printf(
      wp_kses(
        __( '[AltText.ai] Thanks for installing AltText.ai! To complete setup, please <a href="%s">go to the settings page.</a>', 'alttext-ai' ),
        array( 'a' => array( 'href' => array() ) )
      ),
      esc_url(admin_url( 'admin.php?page=atai' ))
    );
